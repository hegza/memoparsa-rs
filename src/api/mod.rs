...
pub fn parse(format: SourceFormat, year: i32, source: &str) {
    match format {
        SourceFormat::Alpha => {
            parse_alpha(year, source);
        }
    }
}
...
where
    P: AsRef<Path>,
{
    let events = parse_alpha(start_year, source);
...
fn parse_alpha(start_year: i32, source: &str) -> Vec<Event> {
